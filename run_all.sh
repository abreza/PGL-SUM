...
train_model () {
    ...
    for split_index in "${split_indexes[@]}"; do
        ...
        if [ $regenerate_pretrained -eq 1 ]; then
            python model/main.py --split_index $split_index --n_epochs $n_epochs --batch_size $batch_size --video_type $video_type --zero_shot --n_pretrain_epochs $n_pretrain_epochs --save_dir $save_dir --save_pretrain_model_dir $pretrain_path
        fi
